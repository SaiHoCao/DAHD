...
**v2 关键改进（相对 v1）**:
- 并行分支从 Medusa 升级为 **Gumiho-style**：输入 `fc(cat(embed(target_next), hidden_t))`，比仅用 `hidden_t` 信息量更丰富
- 三模态路由（原 v1 为二模态）
...
│   │   ├── medusa_baseline.py          # Medusa 并行 baseline
...
│   │   ├── train_medusa_heads.py           # 标准 Medusa Heads 训练
│   │   └── train_dual_branch.py            # 双分支联合训练
│   ├── phase4_e2e/                     # Phase 4: 端到端对比
│   │   ├── run_medusa_e2e_comparison.py    # Medusa 端到端评估
...
│   ├── phase4_medusa/                  # 端到端对比结果
│   │   ├── medusa_e2e_comparison.json      # 完整方法对比数据
│   │   ├── medusa_training_log.json        # Medusa 训练日志
│   │   ├── medusa_comparison_chart.png     # 对比柱状图
...
│   ├── medusa/                             # Medusa 权重
│   │   ├── medusa_best.pt                  # Medusa 最佳权重 (~2.7GB)
│   │   └── medusa_epoch{1,2,3}.pt         # 各 epoch 权重 (~8GB each)
...
3. 后续切换为标准 Medusa 实现（full vocab 151936），彻底避免映射问题

### 3.3 Phase 2b: 训练标准 Medusa Heads

**目标**: 用 full vocab（151936）训练 5 个 Medusa heads，从 Qwen3-8B lm_head 初始化。
...
**目标**: 在相同条件下对比 Vanilla AR / EAGLE-3 / Medusa / DAHD 的端到端性能。
...
| Medusa (Parallel, K=5) | 19.7 | 1.00x | 0.08 |
| DAHD (Router, threshold=0.8) | 22.1 | 1.12x | 0.36 |

**DAHD 路由比例**: Medusa 66.7%, EAGLE-3 33.3%

**对比图表**: `results/phase4_medusa/medusa_comparison_chart.png`
...
- lr_resblock: 1e-3, lr_lm_head: 1e-5
- epochs: 3
...
### 5.2 Parallel Branch: Medusa（标准实现）

- **架构**: 5 个独立 MedusaHead，每个包含 1 个 ResBlock + 共享 lm_head
...
if top1_prob > 0.8:
    draft_tokens = medusa_forward(hidden_states)  # Parallel, K=5
else:
...
| Medusa (K=5) | 标准并行推测解码（5 heads） | `src/drafters/medusa_baseline.py` |
...
| 条件 | 选择 | 原因 |
|------|------|------|
| Easy (top1_prob > 0.8) | Parallel (Medusa) | 1 次前向生成 K 个 token，延迟低 |
...
每一步 speculative decoding 时，先获取 target model 前向的 logits，计算 top-1 概率。若超过阈值 0.8，使用 Medusa 并行生成；否则使用 EAGLE-3 自回归生成。
...
**当前方案**: 模式固定 K
- Parallel: K=5（等于 Medusa heads 数量）
...
| ✅ Router 机制有效 | DAHD (1.12x) > Medusa (1.00x) |
| ✅ EAGLE-3 AR 强劲 | 1.37x speedup, 稳定可靠 |
| ⚠️ Medusa 训练不足 | head_1 仅 12.9%，需要 10-50x 更多训练 |
...
```
在 Easy tokens 上:
  Medusa_acceptance × (1 / parallel_latency) > EAGLE3_acceptance × (1 / ar_latency)
```

即：并行的「低延迟」必须补偿「低精度」。

当前 Medusa head_1 只有 12.9% accuracy → 几乎只能接受 1 个 token（head_0: 81.4%）→ 没有并行优势，因为 EAGLE-3 平均每步也能接受 1 个 token。

**定量分析**:
- EAGLE-3: 平均 1.0 accepted/step, K=5 次 draft forward
- Medusa: 平均 0.08 accepted/step, 1 次 draft forward
- 结论: Medusa 并行优势（1次前向）远不足以弥补极低的 acceptance rate

### 9.3 达成目标需要

1. **Medusa head_1 accuracy > 40-50%**
   - 需要更多训练数据（当前 145K samples，论文标准 10M+ tokens）
   - 需要更多 epochs（当前 3 epochs，建议 20+）
   - head_1 在 epoch 3 已从 1.7% 提升到 12.9%，趋势向好

2. **或使用更强的并行架构**
   - Medusa-v2: 添加 attention layers 到各 head
...
- [ ] 探索 Medusa-v2 架构（各 head 加入 attention 层）
...
# Step 2: 训练 Medusa
python experiments/phase2_architecture/train_medusa_heads.py \
    --hidden_size 4096 \
    --vocab_size 151936 \
    --num_heads 5 \
    --epochs 3 \
    --output_dir checkpoints/medusa
```

### Phase 4: 端到端对比

```bash
python experiments/phase4_e2e/run_medusa_e2e_comparison.py \
    --target_model /mnt/nas1/hf/Qwen3-8B/ \
    --eagle_model /mnt/nas1/hf/qwen3_8b_eagle3/ \
    --medusa_ckpt checkpoints/medusa/medusa_best.pt \
    --num_prompts 50 \
    --max_new_tokens 128 \
    --output_dir results/phase4_medusa
```
