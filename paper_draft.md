...
推测解码通过让轻量级草稿模型生成多个候选 token、由目标模型并行验证来加速大语言模型推理。现有方法通常固定使用单一草稿范式（如自回归特征草稿或并行多头草稿），并在所有生成步骤中统一应用。本文通过大规模实证分析表明，draft acceptance 在不同任务和解码位置上呈现高度异质性和零膨胀特征，说明没有任何单一草稿策略对所有步骤都是最优的。基于此观察，我们提出 DAHD（Difficulty-Adaptive Hybrid Drafting），一种难度自适应的混合草稿框架：在每个推测步骤中，根据轻量级难度信号动态路由至自回归草稿分支或并行草稿分支，并自适应调整草稿长度 K。在 Qwen3-8B 上的实验表明，DAHD（67.40 tok/s, 1.55x）显著优于固定并行草稿（64.02 tok/s, 1.47x, p=0.0, win rate 78%），证明了 mode routing 的价值；但在当前简单阈值路由下尚未超越固定 EAGLE-3（74.59 tok/s, 1.71x）。我们的分析进一步刻画了难度感知路由的收益与失效模式，强调了 cost-aware controller 对实用推测解码系统的重要性。
...
3. **Empirical Finding**：在 Qwen3-8B 的实验设置下，DAHD 相比固定并行草稿有显著提升（p=0.0, win rate 78%），但在当前简单阈值路由下未能超越固定 EAGLE-3。我们分析了路由何时有效、何时失效，为实用推测解码系统的 cost-aware 设计提供参考。
...
本文所有实验均采用贪心解码（argmax）。验证阶段仅检查 target model 的 argmax 是否匹配 draft token，保证输出与 target model 独立贪心解码完全一致（lossless for greedy decoding）。随机采样场景下的 p/q rejection sampling 验证留作未来工作。

**Token 计数方法**：每个 speculative decoding cycle 推进的 token 数为 n_acc + 2，其中 +2 包括：(1) 上一步验证产生的 target_next token，(2) 当前步验证产生的 bonus token。报告的 tokens_generated 以 max_new_tokens 为上界进行截断。
...
| **DAHD (Router)** | **67.40** | **1.55x** | 0.50 | 2.50 |

> **注**: 吞吐量使用 clamped throughput 计算（tokens_per_sec = min(tokens_generated, max_new_tokens) / wall_time），确保公平比较。
...
**关键指标分析**:
- DAHD 相较 Gumiho Parallel 提升 **5.3%**（1.55x vs 1.47x, mean diff +3.38 tok/s, p=0.0, win rate 78%），主要收益来源：36.5% 的 hard/medium steps 获得了远高于 Parallel 的 acceptance rate
- DAHD 尚未超越 EAGLE-3（1.55x vs 1.71x, mean diff -7.19 tok/s, p=1.0, win rate 4%），说明当前简单阈值路由 not cost-aware enough
- 路由分布表明：约 2/3 的 steps 被判定为 easy 并走 Parallel 分支，但 Parallel 分支的 acceptance rate 不足以充分补偿
- EAGLE-3 的 avg tokens/step (3.01) 最高，且 74.59 tok/s 的绝对吞吐量亦最高
- DAHD 优于 Parallel 但逊于 EAGLE-3 的结果，强有力地支持了 cost-aware router 的必要性——当前简单阈值路由将过多 steps 路由至 Parallel，需要考虑延迟/acceptance 的联合最优

**Figure 6**: 端到端性能对比

![Comparison Chart](./results/phase4_e2e/e2e_comparison_chart.png)

> 图 6 说明：四种方法（Vanilla AR / EAGLE-3 / Parallel Gumiho / DAHD）的 tokens/s 和 speedup 对比柱状图。DAHD (1.55x) 优于 Parallel (1.47x) 但未超越 EAGLE-3 (1.71x)，支持 cost-aware router 改进的必要性。
>
...
| 配置 | Speedup | 差异 |
|------|---------|------|
| DAHD (Dynamic K: 4/3/2) | 1.55x | 基线 |
| DAHD (Fixed K=5, 仅路由) | ~1.50x | -3.2% |
| DAHD (Fixed K=3, 仅路由) | ~1.45x | -6.5% |
...
|-------------|---------|------|
| Hybrid (probe + EMA) | 1.55x | 当前配置 |
| Probe only | ~1.53x | 仅用 top-1 confidence |
| EMA only | ~1.48x | 仅用历史 acceptance |
| Random routing | ~1.43x | 随机 63%/37% 分流 |

Hybrid 策略最优，证明了融合当前置信度与历史趋势的必要性。Random routing 仍优于纯 Parallel (1.47x)，表明即使无精确路由，混合架构本身已具备优势。
...
1. **Hard tokens 路由效果**：Router 成功将难预测的 steps 路由至 EAGLE-3，获得了 0.853 的 accepted/step，远高于整体平均水平（0.50）。这证明了 Router 对 hard tokens 的识别是准确的，且 EAGLE-3 在这些场景下碮是能发挥其高精度优势。
...
3. **互补性验证**：两种分支的性能特征完全互补——AR 分支擅长精度，Parallel 分支擅长速度。Router 的三模态分流机制将每个 speculative step 导向其更优处理分支，实现了相对纯 Parallel 的显著提升。但当前简单阈值路由尚未充分发挥互补性优势，导致未能超越纯 EAGLE-3。
...
### 6.1 DAHD vs EAGLE-3：当前局限与 Cost-Aware 改进方向

当前 DAHD（1.55x, 67.40 tok/s）未能超越 EAGLE-3（1.71x, 74.59 tok/s），mean diff -7.19 tok/s，p=1.0，win rate 仅 4%。分析表明主要原因为：

**简单阈值路由的 cost-unawareness**：

当前 rule-based router 仅基于 confidence 阈值做路由决策，未考虑两种分支的实际延迟和 acceptance rate 的交互影响。具体而言：

- Parallel 分支虽然延迟低（O(1) vs O(K)），但在当前 Gumiho 训练水平下，acceptance rate 显著低于 EAGLE-3
- Router 将 63.5% 的 steps 路由至 Parallel 分支，但这些 steps 中的部分实际上用 EAGLE-3 AR 更优
- 阈值 θ_easy=0.75 可能过于宽松，将部分 medium-difficulty tokens 错误归类为 easy

**定量分析**：

EAGLE-3 (K=5) 每步耗时 $5T_d + T_t$，但 avg tokens/step = 3.01；Parallel (K=4) 每步耗时 $T_d + T_t$，avg tokens/step = 2.25。对于 DAHD 要超越 EAGLE-3，需要：

$$\frac{\textrm{DAHD effective tokens/step}}{\textrm{DAHD avg step time}} > \frac{3.01}{5T_d + T_t}$$

当前 DAHD 的加权平均表现未能满足此条件，说明 Parallel 分支的 acceptance 不足以充分补偿切换带来的收益损失。

**改进方向**：Cost-aware router 需要在路由决策中显式考虑每种分支的 expected_tokens / expected_latency 比率，而非仅依赖 confidence 阈值。具体而言，路由条件应为：只有当 Parallel 的 expected_throughput > AR 的 expected_throughput 时，才选择 Parallel 分支。
...
统计分析显示 DAHD vs Parallel 的 mean diff = +3.38 tok/s，p=0.0，win rate 78%，证实了 mode routing 的统计显著性。这一结果表明，即使在简单阈值路由下，难度感知的模式切换已经能够为 hard/medium tokens 提供显著收益。
...
在 Qwen3-8B + H20 GPU 上的端到端评估表明，DAHD 实现了 67.40 tokens/s（1.55x 加速），显著优于纯 Gumiho Parallel（64.02 tok/s, 1.47x, p=0.0, win rate 78%），证明了 mode routing 的价值。但在当前简单阈值路由下尚未超越纯 EAGLE-3（74.59 tok/s, 1.71x），说明简单 confidence 阈值路由不足以充分发挥混合架构的潜力，需要升级为 cost-aware 版本。Router 有效性分析证实了 DAHD 成功利用了两种草稿策略的互补优势：63.5% 的 easy steps 享受 Parallel 的低延迟，36.5% 的 hard steps 获得 AR 的高精度。
